...
namespace Mapbox.Unity.Map
{
    ...
    public class AbstractMap : MonoBehaviour, IMap
    {
        ...
        private Vector3 GeoToWorldPositionXZ(Vector2d latitudeLongitude)
        {
            ...
            var worldPos = Conversions.GeoToWorldPosition(latitudeLongitude, CenterMercator, scaleFactor).ToVector3xz();
        ...
        ///<summary>
        ///Fixes VR issue where map scale is not 1:1 with real-life
        ///</summary>
        void FixMapScale()
        {
            float verticalScale = 1 / Mathf.Cos(Mathf.Deg2Rad * (float)this.CenterLatitudeLongitude.x);
            this.transform.localScale = new Vector3(1f, verticalScale, 1f);
        }

        void FixBuildings()
        {
			IEnumerable<VectorSubLayerProperties> newData =_vectorData.GetAllFeatureSubLayers();
        }
